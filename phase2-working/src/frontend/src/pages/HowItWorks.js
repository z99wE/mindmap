...
export function HowItWorks() {
  ...
  const phone2Messages = [
    ...
    { text: 'Umbrella — 72% rain today', time: '8:45 AM', icon: 'cloud' },
  ...
  const phone4Messages = [
    ...
    { type: 'thought-node', theme: 'Health', color: 'var(--md-sys-color-primary)', text: 'Call Dr. Mehta about blood test', urgency: 'critical', halfLife: '4h' },
    { type: 'thought-node', theme: 'Health', color: 'var(--md-sys-color-primary)', text: 'Schedule annual checkup', urgency: 'normal', halfLife: '72h' },
    { type: 'thought-node', theme: 'Finance', color: 'var(--md-sys-color-tertiary)', text: 'Pay electricity bill — due today', urgency: 'high', halfLife: '2h' },
    { type: 'navigate', text: 'Navigating: 3 themes to review. Swipe through your route.', step: '1 / 8' },
  ];

  const phone5Messages = [
    { type: 'geo-alert', icon: 'location_away', color: 'var(--md-sys-color-tertiary)', title: 'Leaving Home Zone', text: 'Departure detected at 8:42 AM', time: '8:42 AM' },
  ...
  const phone6Messages = [
    ...
    { text: 'Your API key for Groq is missing. Add it in settings to enable LLM features.', time: 'System Alert', icon: 'warning' },
  ...
  function renderBubble(msg, style) {
    const tierColors = { 1: 'var(--color-success)', 2: 'var(--color-analytical)', 3: 'var(--md-sys-color-error)' };
  ...
  function renderMapMindMsg(msg) {
    ...
    if (msg.type === 'thought-node') {
      const urgColor = msg.urgency === 'critical' ? 'var(--md-sys-color-error)' : msg.urgency === 'high' ? 'var(--color-analytical)' : 'rgba(235,235,235,.4)';
    ...
    if (msg.type === 'navigate') {
      return `<div style="padding:0.6rem 0.75rem;border-radius:12px;background:rgba(204,255,0,0.15);border:1px solid rgba(204,255,0,0.3);">
        <div style="display:flex;justify-content:space-between;align-items:center;">
          <span style="font:600 11px/1.3 system-ui;color:var(--md-sys-color-primary);">${msg.text}</span>
  ...
  function renderGeoFenceMsg(msg) {
    ...
    if (msg.type === 'brief') {
      return `<div style="padding:0.6rem 0.75rem;border-radius:12px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);">
        <div class="mono-label" style="color:var(--md-sys-color-primary);font-size:9px;margin-bottom:6px;text-transform:uppercase;">${msg.heading}</div>
    ...
    if (msg.type === 'nearby') {
      return `<div style="padding:0.6rem 0.75rem;border-radius:12px;background:rgba(204,255,0,0.08);border:1px solid rgba(204,255,0,0.2);">
        <div style="display:flex;align-items:center;gap:0.4rem;margin-bottom:6px;">
          <span class="mono-label" style="color:var(--md-sys-color-primary);font-size:9px;">${(msg.icon || 'NEAR').toUpperCase()}</span>
          <span style="font:600 11px/1.3 system-ui;color:var(--md-sys-color-secondary);">${msg.text}</span>
        </div>
