...
const OptionWheel = ({
  ...
  const runFrame = useCallback(now => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      // Clamp the distance used for blur so far (nearly invisible) options
      // don't pay for huge per-frame blur compositing.
      el.style.filter = cfg.blur > 0 ? `blur(${(Math.min(dist, 3) * cfg.blur).toFixed(2)}px)` : 'none';
