...
const RESEND_SENDER_NAME = process.env.RESEND_SENDER_NAME || 'ReMentally';
...
async function sendEmail({ to, toName, subject, htmlContent }) {
  ...
  console.log('[Mailer] Email sent via ReMentally mailer to', to, '| messageId:', data.id);
  return data;
}
...
function waitlistTemplate({ name, plan }) {
  ...
  const bodyText = isUpdates
    ? `Thank you for subscribing to updates and news from <strong style="color:#ccff00;">ReMentally</strong>. We'll keep you in the loop with new features, cognitive science deep-dives, and product releases.`
    : `We've reserved your spot for <strong style="color:#ccff00;">${planLabel}</strong>. You'll be among the first to experience ReMentally when we open access.`;
    ...
    <tr><td align="center">
      <table width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;">
        ...
        <tr><td style="background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.10);border-radius:20px;padding:40px 36px;">
          ...
          <a href="https://rementally.onrender.com" style="display:inline-block;background:linear-gradient(145deg,#d6ff3e,#ccff00);color:#000;font:700 14px/1 Inter,system-ui;padding:14px 28px;border-radius:100px;text-decoration:none;">Try the Free Tier &#x2192;</a>
        </td></tr>
        <tr><td style="padding-top:24px;text-align:center;">
          <p style="margin:0;font:400 12px/1.6 Inter;color:rgba(240,244,238,0.3);">ReMentally &#x2014; Your cognitive coprocessor<br>You're receiving this because you joined the waitlist.</p>
        </td></tr>
      </table>
    </td></tr>
  </table>
...
function welcomeTemplate({ firstName, email }) {
  ...
  return `<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"><title>Welcome to ReMentally</title></head>
<body style="margin:0;padding:0;background:#080a0f;font-family:'Inter',system-ui,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#080a0f;padding:40px 16px;">
    <tr><td align="center">
      <table width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;">
        ...
        <tr><td style="background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.10);border-radius:20px;padding:40px 36px;">
          ...
          <p style="margin:0 0 24px;font:400 16px/1.6 Inter;color:rgba(240,244,238,0.7);">Your ReMentally account is ready. Start capturing thoughts and letting your cognitive coprocessor do the heavy lifting.</p>
          <a href="https://rementally.onrender.com" style="display:inline-block;background:linear-gradient(145deg,#d6ff3e,#ccff00);color:#000;font:700 14px/1 Inter;padding:14px 28px;border-radius:100px;text-decoration:none;">Open ReMentally &#x2192;</a>
        </td></tr>
        <tr><td style="padding-top:24px;text-align:center;">
          <p style="margin:0;font:400 12px/1.6 Inter;color:rgba(240,244,238,0.3);">ReMentally &#x2014; Your cognitive coprocessor</p>
        </td></tr>
      </table>
    </td></tr>
  </table>
...
async function sendWaitlistConfirmation({ email, name, plan }) {
  return sendEmail({
    ...
    subject: `You're on the ReMentally waitlist 🧠`,
...
async function sendWelcomeEmail({ email, firstName }) {
  return sendEmail({
    ...
    subject: `Welcome to ReMentally, ${firstName || email.split('@')[0]}!`,
